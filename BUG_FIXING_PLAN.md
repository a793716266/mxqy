...
- **AI/怪物无物理碰撞**：
  - 禁用 `field-movement.js` 中 follower（AI 队友）的防重叠碰撞推挤（`_updateFollowers` 内原 388-420 行的队友↔队友、队友↔主角互相推开逻辑），避免 AI 被互相推挤或被主角挤到角落。
  - 确认野外场景 AI 队友移动（`_updateFollowers`）、怪物更新（`_updateMonsters`）均不做碰撞检测；被控者与怪物的 `_checkMonsterCollision`（接触开战）调用已在 `field-scene.js` 第 847 行移除，仅保留与地图障碍物的地形碰撞（`_checkObstacleCollision`）。AI 角色与怪物互不影响、互不阻挡。
- **修改文件**：
  - `scripts/scenes/field-scene.js`：`init` 新增 `aiRecall`、`_updateFollowers` 独立行动/召回分支、`_findNearestMapMonster`、`_recallAlliesToPlayer`、`_handleTap` 召回按钮、`render` 召回按钮绘制（**已调整至右上角顶栏下方醒目位置，避开顶栏黑条与角色卡片遮挡**）
  - `scripts/systems/field-battle-system.js`：`_updateAllyAI`（CD 倒计时 + MP 回复 + 技能尝试 + 远程普攻投射物）、新增 `_allyTryCastSkill` / `_allyCastBladeStorm`
...
- **问题**：怪物攻击时显示 `ReferenceError: dt is not defined`，导致游戏崩溃
- **原因**：在 `_renderCatMonster` 方法中重复添加了攻击动画更新逻辑，使用了未定义的 `dt` 变量
- **分析**：
  1. 攻击动画帧更新逻辑已经在 `_updateMonsters(dt)` 方法中正确实现（第 812-830 行）
  2. 在 `_renderCatMonster` 方法中错误地重复添加了更新逻辑（第 2197-2204 行）
  3. 渲染方法中无法访问 `dt` 变量，导致 `ReferenceError`
- **修复方案**：
  1. 删除 `_renderCatMonster` 中的攻击动画更新逻辑（第 2191-2210 行）
  2. 只保留动画类型选择逻辑（`attack`/`walk`/`idle`）
  3. 攻击动画帧更新完全由 `_updateMonsters(dt)` 负责
- **修改文件**：
  - `scripts/scenes/field-scene.js`:
    - 删除 `_renderCatMonster` 中的攻击动画帧更新逻辑
    - 保留动画类型选择逻辑
- **测试说明**：
  1. 进入阳光草原，靠近怪物
  2. 怪物攻击时应该播放攻击动画，不再崩溃
  3. 攻击动画帧应该正确更新（已在 `_updateMonsters` 中处理）
- **提交信息**：`fix: 修复野外怪物攻击动画导致的 dt 未定义错误`
...
- **问题**：史莱姆猫卡在 `moving_to_attack` 状态，距离减少很慢（423.6→414.8）
- **分析**：
  1. `MELEE_RANGE = 80 * dpr`，如果 dpr=3，则 `contactDist = 216` 像素
  2. 敌人需要移动约207像素才能到达攻击范围
  3. 敌人移动速度 = 120 像素/秒，需要约1.7秒
  4. 如果目标（英雄）在移动，敌人可能一直无法到达攻击范围

- **修改**：在 `moving_to_attack` 状态的调试日志中，显示敌人位置、目标位置、移动速度、effectiveDt
- **修改文件**：
  - `battle-combat.js`:
    - 修改 `moving_to_attack` 状态的调试日志，添加更多信息
...
- **测试说明**：
  ...
  2. 观察调试日志，查看：
     ...
     - `eSpeed2`：每帧移动距离
     - `effectiveDt`：帧时间
  3. 如果 `eSpeed2` 很小（如0.5），说明敌人移动速度很慢
  4. 如果 `targetPos` 在变化，说明目标在移动，导致敌人一直无法到达攻击范围
...
- **问题**：用户反馈多只暗影鼠的情况下，攻击速度很快，怀疑代码有逻辑错误
- **修改**：在调试日志中显示 `enemy.id`，方便区分多只暗影鼠
- **修改**：显示 `battleSpeed` 的值，方便诊断加速模式问题
- **修改文件**：
  - `battle-combat.js`:
    - 修改 `_updateEnemyAutoAttack`，在调试日志中显示 `enemy.id` 和 `battleSpeed`

- **测试说明**：
  1. 进入战斗，打开浏览器控制台（F12）
  2. 观察调试日志，确认每只暗影鼠有独立的 `attackTimer`
  3. 确认 `battleSpeed` 的值是否正确
  4. 如果多只暗影鼠的 `attackTimer` 是独立的，那么代码逻辑正确
...
- **问题1**：暗影鼠的普攻攻击速度过快，感觉可以疯狂攻击
  - 根本原因：`_getEnemyAttackInterval` 函数的最低攻击间隔是0.8秒，对高速敌人来说太快
  - 修复方案：调整攻击间隔公式为1.2-3.0秒，高速敌人最低1.2秒

- **问题2**：暗影鼠的技能"暗影之咬"没有计算CD
  - 根本原因：`SKILL_CD_TABLE` 中没有 `jump_attack` 类型，导致 `lookupBaseCd` 返回默认值5
  - 修复方案：在 `SKILL_CD_TABLE` 中添加 `jump_attack` 类型条目

- **修改文件**：
  - `battle-combat.js`:
    - 修改 `_getEnemyAttackInterval`，调整攻击间隔公式
    - 修改 `SKILL_CD_TABLE`，添加 `jump_attack` 类型

- **测试说明**：
  1. 进入战斗，观察暗影鼠的普攻速度，应该明显变慢
  2. 观察暗影鼠的技能"暗影之咬"，应该有CD（基础8秒）
  3. 打开浏览器控制台（F12），查看调试日志，确认CD状态

### 2026-05-17（22:00）

**fix: 修复艾米非治愈冲击技能错误使用skill动画的问题**

- **问题**：艾米的非治愈冲击技能（如治愈之爪、生命波纹）错误地使用了 `aimi/skill/` 动画目录
- **根本原因**：在 `_updateCombatUnits` 函数中，`isSkillAttack` 分支没有对艾米进行特殊处理
- **修复方案**：在 `isSkillAttack` 分支中添加对艾米的特殊处理，只有技能名称为'治愈冲击'时才使用 'skill' 动画
- **修改文件**：
  - `battle-combat.js`:
    - 修改 `_updateCombatUnits`，添加对艾米的特殊处理

- **测试说明**：
  1. 进入战斗，让艾米使用治愈之爪技能
  2. 确保使用的是 `aimi/attack/` 动画目录，而不是 `aimi/skill/`
  3. 确保治愈冲击技能仍然使用 `aimi/skill/` 动画目录
  4. 打开浏览器控制台（F12），查看调试日志，确认动画类型正确
...
- **问题**：用户报告BUFF技能（如暗影突袭、圣盾之光）无CD，可重复释放
  - 可能的原因：技能CD设置逻辑有问题，或者 `isSpecialSkill` 判断错误
  - 需要添加调试日志，追踪CD设置的每一步

- **修改内容**：
  1. 在 `_executeEnemyAttackAnim` 中添加调试日志
     - 输出 `skill.id`、`skill.name`、`isSpecialSkill`、`timer` 是否存在
     - 如果CD已设置，输出 `✅ CD已设置`
     - 如果CD未设置，输出 `❌ CD未设置`，并显示原因

- **修改文件**：
  - `battle-combat.js`: 添加技能CD设置调试日志

- **测试说明**：
  1. 打开浏览器控制台（F12），查看调试日志
  2. 让敌人使用BUFF技能（如暗影突袭、圣盾之光）
  3. 查看控制台输出：
     - 如果看到 `✅ CD已设置`，说明CD设置成功
     - 如果看到 `❌ CD未设置`，说明有问题，请根据日志排查
  4. **请截图控制台日志发给开发者**

- **下一步排查**：
  1. 如果CD设置成功，但技能仍然无CD，可能是**CD倒计时逻辑有问题**
  2. 如果CD设置失败，可能是 `isSpecialSkill` 判断错误
  3. 检查 `_updateEnemyAutoAttack` 中的CD倒计时逻辑
...
- **问题1**：暗影突袭等BUFF技能无限使用
  - 根本原因：BUFF技能动画播放完后没有清理 `enemyAttacking` 状态
  - 修复方案：
    1. 在 `_executeEnemyAttackAnim` 中为BUFF技能设置 `onAttackComplete` 回调
    2. 回调中清理 `enemyAttacking` 状态，重置 `phase` 为 `auto_battle`
    3. 立即调用 `_applyEnemyBuff` 应用BUFF效果（如暗影突袭的隐身）
    
- **问题2**：敌人死亡后其他怪物无法攻击，动画资源异常
  - 根本原因：死亡敌人的攻击请求还在队列中，导致状态混乱
  - 修复方案：
    1. 在 `_executeEnemyAttackAnim` 开头添加敌人死亡检查
    2. 如果敌人已死亡（hp <= 0），立即清理状态并返回
    3. 添加敌人AI的CD倒计时调试日志

- **修改文件**：
  - `battle-combat.js`: 
    - 修复BUFF技能状态清理（添加 `onAttackComplete` 回调）
    - 添加敌人死亡检查（防止攻击已死亡敌人）
    - 添加CD状态调试日志（方便排查问题）
    
- **测试说明**：
  1. 打开浏览器控制台（F12），查看调试日志
  2. 召唤多只暗影鼠，观察暗影突袭是否有CD
  3. 打死暗影鼠后，观察其他敌人是否能正常攻击
  4. 如果还有问题，请截图控制台日志发给开发者

### 2026-05-17（00:15）

**fix: 修复敌人技能无限使用BUG（暗影突袭无CD）**
- **问题**：当召唤了3只或3只以上的暗影鼠，暗影鼠就能无限使用暗影突袭（BUFF技能），导致怪物其他状态全失效
- **根本原因**：敌人技能CD没有被正确设置
  1. `_executeEnemyAttackAnim` 中的CD设置条件 `skill.id !== undefined` 对于没有 `id` 字段的技能（如暗影突袭）会返回 `false`
  2. 敌人技能CD倒计时逻辑缺失，导致CD设置后不会减少
- **修复方案**：
  1. 修改 `_executeEnemyAttackAnim` 中的CD设置条件，同时检查 `skill.id` 和 `skill.name`
  2. 在 `_updateEnemyAutoAttack` 函数中添加敌人技能CD倒计时逻辑
- **修改文件**：
  - `battle-combat.js`: 修复CD设置条件，添加CD倒计时逻辑
- **测试建议**：召唤多只暗影鼠，观察暗影突袭是否现在有CD
...
**feat: 闯关模式(塔防)完整实现**
- **10波固定波次系统**：从水晶位置生成，渐进难度（史莱姆猫→暗影鼠→兽人→混合大军）
- **水晶可攻击状态机制**：初始不可攻击(isAttackable=false)，10波全灭后变为可攻击
- **水晶视觉改造**：不可攻击时暗灰色+🛡盾牌图标，可攻击时红色闪烁+⚔跳动箭头指引
- **怪物AI重构**：
  - 史莱姆猫→远程投射物+技能(黏液喷射/包裹，25%-40%概率冻结)
  - 暗影鼠→快速近战(85px/s移速)
  - 远程怪物保持距离AI（太近后退，太远靠近）
- **角色目标判定**：水晶不可攻击时不作为攻击目标（近战/法师均受保护）
- **UI波次进度显示**：顶部中央波次号+进度条+存活怪数
- **Bug修复**：
  - 投射物 `onHit()` 未传参 → `Cannot read property 'dmg' of undefined`
  - 波次编号日志不一致(开始用++后值，完成用-1)
  - 波次逻辑改为当前波全灭后才进入下一波
...
- 提交ID：3c768ea
- 提交信息：feat: 战斗系统多项优化
- 提交时间：2026-04-08 20:54

**修复1：敌人攻击动画被角色头像遮挡**

**问题现象：**
- 史莱姆猫攻击时的动画被己方角色头像覆盖，看不到敌人

**根本原因：**
- 渲染顺序问题：`_renderAttackingEnemy` 在 `_renderParty` 之前绘制，导致己方队伍覆盖敌人动画

**解决方案：**
- 调整渲染顺序：敌人UI → 己方队伍 → 敌人精灵/攻击中敌人 → 攻击中角色
- 将攻击中敌人的绘制移到己方队伍之后

**修复2：史莱姆猫攻击/技能动画速度过慢**

**问题现象：**
- 史莱姆猫攻击动画播放时间过长（attack 1.6秒，skill 3.1秒）

**解决方案：**
- 跳帧播放：每2帧取1帧（step=2），动画时长减半
- 帧率提升：100ms/帧 → 55ms/帧
- attack: 16帧→8帧显示 × 55ms = 0.44秒
- skill: 31帧→16帧显示 × 55ms = 0.88秒

**修复3：近战攻击目标选择错误**

**问题现象：**
- 臻宝选择攻击某个敌人时，伤害却打到了第一个敌人（主敌人）

**根本原因：**
- `_updateAttackAnimation` 的 hit 阶段硬编码使用 `this.enemy`（主敌人），而非玩家选择的目标

**解决方案：**
- 新增 `this.attackAnimTarget` 保存实际攻击目标
- 伤害结算使用 `this.attackAnimTarget || this.enemy`
...
**解决方案：**
- 目标位置从 `targetPos.x + 40 * dpr`（偏右）改为 `targetPos.x - 30 * dpr`（偏左）

**修复5：地图普通怪物命名统一**

**问题现象：**
- 地图上显示各种不同的怪物名称（野猫、史莱姆猫、暗影鼠等）

**解决方案：**
- 非Boss/非精英的普通怪物名称统一为「坏猫」
- Boss和精英保留各自原名

**优化6：野猫战斗场景添加idle帧动画**

**问题现象：**
- 野猫在战斗场景中使用默认圆形绘制，没有动画

**解决方案：**
- 使用 `cat_idle` 目录的16帧动画作为野猫战斗idle动画
- 野猫只有idle帧，攻击时走默认跳跃攻击逻辑
- 帧尺寸：816x1120，120ms/帧
