...
def process_ice_shard_hit():
    """处理冰晶术击中特效帧"""
    # 路径设置
    base_dir = Path(__file__).parent.parent.parent
    input_dir = base_dir / 'images/characters_anim/ice_shard_frames'
    output_dir = base_dir / 'images/effects/ice_shard_hit'
    
    # 创建输出目录
    output_dir.mkdir(parents=True, exist_ok=True)
    
    print(f"[Ice Shard Hit] 处理冰晶术击中动画...")
    print(f"  输入: {input_dir}")
    print(f"  输出: {output_dir}")
    
    # 处理 frame_0045 到 frame_0065
    for i in range(45, 66):  # 45 到 65
        input_file = input_dir / f'frame_{i:04d}.png'
        output_file = output_dir / f'ice_shard_hit_{i-44:02d}.png'
    ...
    # 计算总大小
    total_size = sum(
        os.path.getsize(output_dir / f'ice_shard_hit_{i:02d}.png') 
        for i in range(1, 22) 
        if (output_dir / f'ice_shard_hit_{i:02d}.png').exists()
    ) / (1024 * 1024)
    
    print(f"\n✅ 处理完成！总大小: {total_size:.2f} MB")

if __name__ == '__main__':
    process_ice_shard_hit()
