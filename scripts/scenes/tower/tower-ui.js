...
function render(ctx, battle) {
  ...
  // ★ 相机震动：用 sin+cos 生成确定性偏移，避免每帧随机导致画面撕裂
  const shakeAmp = battle.camera.shakeX || 0
  const shakeAmpY = battle.camera.shakeY || 0
  if (shakeAmp > 0.1 || shakeAmpY > 0.1) {
    const t = battle.battleTime * 0.05
    ctx.translate(Math.sin(t * 7.3) * shakeAmp, Math.cos(t * 11.1) * shakeAmpY)
  }
...
function renderBottomPanel(ctx, battle) {
  ...
  const skillBarH = Math.max(H * 0.055, 40)
  const tacticsBarH = Math.max(H * 0.06, 42)
  const charBarH = Math.max(H * 0.09, 56)  // 角色切换栏（替代原装备栏）
  const totalBarH = tacticsBarH + skillBarH + charBarH + 8
  ...
  renderCharSwitchBar(ctx, 0, panelY + skillBarH + 4 + tacticsBarH + 4, W, charBarH, battle)

  // 背包弹出层（覆盖在战斗区域上方）
  if (battle._inventoryOpen) {
    renderInventoryPanel(ctx, battle)
  }
  ...
  battle._bottomPanelBounds = {
    ...
    charBar: { x: 0, y: panelY + skillBarH + 4 + tacticsBarH + 4, w: W, h: charBarH },
  }
}
...
// ----------------------------------------------------------------
// renderCharSwitchBar: bottom char avatar switch bar + backpack btn
// ----------------------------------------------------------------
function renderCharSwitchBar(ctx, panelX, panelY, panelW, charBarH, battle) {
  const dpr = battle.dpr || 1;
  var charData = battle.party || [];
  var selectedIdx = battle.selectedCharIndex || 0;

  battle._charAvatarBtns = [];
  battle._backpackBtn = null;

  var btnH = (charBarH - 8) * dpr;
  var btnW = btnH;
  var gap = 8 * dpr;
  var totalW = charData.length * btnW + (charData.length - 1) * gap;
  var startX = panelX + (panelW - totalW) / 2;
  var btnY = panelY + 4 * dpr;
  ...
  charData.forEach(function(char, idx) {
    ...
    ctx.beginPath();
    ctx.arc(bx + btnW / 2, btnY + btnH / 2, btnW / 2, 0, Math.PI * 2);
    var grad = ctx.createRadialGradient(bx + btnW / 2, btnY + btnH / 2, 0, bx + btnW / 2, btnY + btnH / 2, btnW / 2);
    if (isSelected) {
    ...
    ctx.textAlign = 'center';
    ctx.textBaseline = 'middle';
    ctx.fillText(char.name ? char.name[0] : '' + (idx + 1), bx + btnW / 2, btnY + btnH / 2);

    if (isSelected) {
      ctx.fillStyle = '#FFD700';
      ctx.fillRect(bx + btnW * 0.2, btnY + btnH - 5, btnW * 0.6, 4);
    }

    battle._charAvatarBtns.push({ x: bx, y: btnY, w: btnW, h: btnH, index: idx });
  });

  var bpBtnW = 80 * dpr;
  var bpBtnH = (charBarH - 8) * dpr;
  var bpBtnX = panelX + panelW - bpBtnW - 12 * dpr;
  var bpBtnY = btnY;

  ctx.fillStyle = 'rgba(50, 50, 80, 0.9)';
  roundRect(ctx, bpBtnX, bpBtnY, bpBtnW, bpBtnH, 8 * dpr);
  ctx.fill();
  ctx.strokeStyle = '#888';
  ctx.lineWidth = 1.5;
  ctx.stroke();
  ...
  ctx.fillStyle = '#FFD700';
  ctx.font = Math.floor(bpBtnH * 0.35) + 'px sans-serif';
  ctx.textAlign = 'center';
  ctx.textBaseline = 'middle';
  ctx.fillText('Backpack', bpBtnX + bpBtnW / 2, bpBtnY + bpBtnH / 2);

  battle._backpackBtn = { x: bpBtnX, y: bpBtnY, w: bpBtnW, h: bpBtnH };
}

// ----------------------------------------------------------------
// renderInventoryPanel: popup inventory (char card + 4x4 grid)
// ----------------------------------------------------------------
function renderInventoryPanel(ctx, battle) {
  var H = ctx.canvas ? ctx.canvas.height : (typeof gameHeight !== 'undefined' ? gameHeight : 667);
  var invData = { items: (battle.inventory || []) };
  const dpr = battle.dpr || 1;
  var charData = battle.party || [];
  var selectedIdx = battle.selectedCharIndex || 0;
  var invItems = invData.items || [];
  var char = charData[selectedIdx] || {};

  var S = battle.width || 375;
  var margin = 20 * dpr;
  var pw = S - margin * 2;
  var colCount = 4;
  var cellSize = 52 * dpr;
  var cellGap = 6 * dpr;
  var totalInvW = colCount * cellSize + (colCount - 1) * cellGap;
  var invPadding = (pw - totalInvW) / 2;

  var charCardH = 90 * dpr;
  var charCardY = H * 0.35;
  var invStartY = charCardY + charCardH + 16;
  var panelH = invStartY - charCardY + colCount * cellSize + (colCount - 1) * cellGap + invPadding + 50;
  var panelX = margin;
  var panelY = charCardY - 20;

  ctx.fillStyle = 'rgba(15, 15, 30, 0.95)';
  roundRect(ctx, panelX, panelY, pw, panelH, 16 * dpr);
  ctx.fill();
  ...
  // char card
  var ccX = panelX + invPadding;
  var ccY = charCardY;
  var ccW = totalInvW;
  var ccH = charCardH;

  ctx.fillStyle = 'rgba(40, 40, 60, 0.8)';
  roundRect(ctx, ccX, ccY, ccW, ccH, 10 * dpr);
  ctx.fill();
  ...
  var avaR = 32 * dpr;
  var avaX = ccX + avaR + 12 * dpr;
  var avaY = ccY + ccH / 2;
  ctx.beginPath();
  ctx.arc(avaX, avaY, avaR, 0, Math.PI * 2);
  ctx.lineWidth = 2 * dpr;
  var avaGrad = ctx.createRadialGradient(avaX, avaY, 0, avaX, avaY, avaR);
  avaGrad.addColorStop(0, '#FFD700');
  avaGrad.addColorStop(1, '#FF8C00');
  ctx.fillStyle = avaGrad;
  ctx.fill();
  ctx.strokeStyle = '#fff';
  ctx.lineWidth = 2;
  ctx.stroke();
  ctx.fillStyle = '#fff';
  ctx.font = 'bold ' + Math.floor(avaR * 0.8) + 'px sans-serif';
  ctx.textAlign = 'center';
  ctx.textBaseline = 'middle';
  ctx.fillText(char.name ? char.name[0] : '' + (selectedIdx + 1), avaX, avaY);

  var infoX = avaX + avaR + 14;
  ctx.fillStyle = '#FFD700';
  ctx.font = 'bold ' + Math.floor(15 * dpr) + 'px sans-serif';
  ctx.textAlign = 'left';
  ctx.textBaseline = 'top';
  ctx.fillText(char.name || ('Char' + (selectedIdx + 1)), infoX, ccY + 12);
  ctx.fillStyle = '#aaa';
  ctx.font = Math.floor(12 * dpr) + 'px sans-serif';
  ctx.fillText('Lv.' + (char.level || 1) + '  ' + (char.job || 'Warrior'), infoX, ccY + 34);

  var hpBarW = ccW - (infoX - ccX) - 12;
  var hpBarH = 8 * dpr;
  var hpBarX = infoX;
  var hpBarY = ccY + ccH - 24;
  var hpPct = char.hp !== undefined ? Math.max(0, Math.min(1, char.hp / (char.maxHp || 100))) : 1;
  ctx.fillStyle = 'rgba(0,0,0,0.5)';
  roundRect(ctx, hpBarX, hpBarY, hpBarW, hpBarH, 4 * dpr);
  roundRect(ctx, hpBarX, hpBarY, hpBarW * hpPct, hpBarH, 4 * dpr);
  ctx.fill();
  ctx.fillStyle = hpPct > 0.5 ? '#4CAF50' : hpPct > 0.25 ? '#FF9800' : '#F44336';
  roundRect(ctx, hpBarX, hpBarY, hpBarW * hpPct, hpBarH, 4);
  ctx.fill();

  // inventory grid
  var gridY = invStartY;
  battle._invSlotBounds = [];
  battle._invCloseBtn = null;

  for (var i = 0; i < 16; i++) {
    var col = i % colCount;
    var row = Math.floor(i / colCount);
    var sx = panelX + invPadding + col * (cellSize + cellGap);
    var sy = gridY + row * (cellSize + cellGap);
    var item = invItems[i];

    ctx.fillStyle = 'rgba(60, 60, 80, 0.8)';
    roundRect(ctx, sx, sy, cellSize, cellSize, 6 * dpr);
    ctx.fill();
    ctx.strokeStyle = item ? '#666' : 'rgba(100,100,100,0.3)';
    ctx.lineWidth = 1;
    ctx.stroke();
    ...
    if (item) {
      ...
      ctx.textAlign = 'center';
      ctx.textBaseline = 'middle';
      ctx.fillText(item.icon || '[item]', sx + cellSize / 2, sy + cellSize / 2 - 6);
      if (item.count > 1) {
        ctx.fillStyle = '#FFD700';
        ctx.font = 'bold 10px sans-serif';
        ctx.textAlign = 'right';
        ctx.textBaseline = 'bottom';
        ctx.fillText(item.count, sx + cellSize - 3, sy + cellSize - 3);
      }
    }

    battle._invSlotBounds.push({ x: sx, y: sy, w: cellSize, h: cellSize, index: i });
  }

  // close btn
  var closeW = 60 * dpr;
  var closeH = 28 * dpr;
  var closeX = panelX + pw - closeW - 8;
  var closeY = panelY + 10;
  ctx.fillStyle = 'rgba(200, 50, 50, 0.8)';
  roundRect(ctx, closeX, closeY, closeW, closeH, 6 * dpr);
  ctx.fill();
  ctx.fillStyle = '#fff';
  ctx.font = Math.floor(12 * dpr) + 'px sans-serif';
  ctx.textAlign = 'center';
  ...
  battle._invCloseBtn = { x: closeX, y: closeY, w: closeW, h: closeH };
}
...
function handleUITap(battle, x, y) {
  ...
  // close inventory popup btn
  if (battle._invCloseBtn && x >= battle._invCloseBtn.x && x <= battle._invCloseBtn.x + battle._invCloseBtn.w && y >= battle._invCloseBtn.y && y <= battle._invCloseBtn.y + battle._invCloseBtn.h) {
    battle._inventoryOpen = false
    return
  }
  // avatar switch character
  if (battle._charAvatarBtns) {
    for (var ai = 0; ai < battle._charAvatarBtns.length; ai++) {
      var ab = battle._charAvatarBtns[ai]
      if (x >= ab.x && x <= ab.x + ab.w && y >= ab.y && y <= ab.y + ab.h) {
        battle.selectedCharIndex = ab.index
        console.log("[Tower] Switched to char " + ab.index)
        return
      }
    }
  }
  // backpack btn toggle
  if (battle._backpackBtn && x >= battle._backpackBtn.x && x <= battle._backpackBtn.x + battle._backpackBtn.w && y >= battle._backpackBtn.y && y <= battle._backpackBtn.y + battle._backpackBtn.h) {
    battle._inventoryOpen = !battle._inventoryOpen
    return
  }
  // 技能按钮
