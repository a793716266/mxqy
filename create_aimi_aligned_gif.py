...
# 配置
INPUT_BASE = 'subpackages/battle/images/characters_anim/transparent/aimi_aligned_final'
OUTPUT_DIR = 'subpackages/battle/images/characters_anim/transparent/aimi_aligned_preview'
...
def create_animation_gif(anim_name, duration=200):
    ...
    # 保存为GIF
    output_path = os.path.join(OUTPUT_DIR, f'{anim_name}_aligned.gif')
...
def create_combined_preview():
    ...
    # 保存
    output_path = os.path.join(OUTPUT_DIR, 'all_animations_aligned.png')
    canvas.save(output_path)
    print(f'  ✅ 已保存: {output_path}')

def main():
    print('=' * 60)
    print('创建对齐后的艾米动画GIF预览')
    print('=' * 60)
    ...
    print('=' * 60)
    print('\n预览文件：')
    print(f'  - 单个动画GIF: {OUTPUT_DIR}/[动画名]_aligned.gif')
    print(f'  - 组合预览图: {OUTPUT_DIR}/all_animations_aligned.png')
    print('\n如何反馈问题：')
